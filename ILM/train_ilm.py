...
def doc_and_char_masks_to_input_and_tt(
    ...
    if skip_naive_incomplete:
        contexts_and_answers = [(m, (c, a)) for m, (c, a) in contexts_and_answers if
                                (len(c) + 1 + len(doc_tokens_ids) + 1) <= sequence_length]
    ...
    for i, (mask, (context, answers)) in enumerate(contexts_and_answers):
        ...
        if task in [Task.NAIVE, Task.LM, Task.REVERSE_LM]:
            ...
            for (_, tok_off, tok_len) in mask:
                ...
                tts[i,
                context_len + 1 + tok_off + tok_len:context_len + 1 + tok_off + tok_len + 1] = TargetType.INFILL_SPECIAL.value
        else:
    ...
    if args.eval_only:
        ...
        for i, eval_batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                eval_logits, _ = model(eval_inputs, return_dict=False)
        ...
        while True:
            ...
            for batch in train_dataloader:
                ...
                logits, _ = model(inputs, return_dict=False)
...
if __name__ == '__main__':
    ...
    tokenizer_args.add_argument('--tokenizer_name', type=str,
